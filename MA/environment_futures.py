...
class TradingEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.contract_positions = [2] * contract_dim
        self.deposits = [float(54000),  # self.contract_positions[0]*self.margins[0],
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            if self.episodes % self.print_verbosity == 0:
                ...
                print(f'steps done: {self.point_in_time}')
            ...
            step_start_assets = self.state[0] + sum(
                np.array(self.state[1:(self.contract_dim + 1)]) * self.contract_positions)
            ...
            # print('prices:', np.array(self.state[1:(self.contract_dim + 1)]))
    ...
    def reset(self) -> np.array:
        ...
        self.point_in_time = np.random.randint(len(self.df.index.unique()) - 1)  # 0
        self.data = self.df.loc[self.point_in_time, :]
        self.costs = 0
        self.trades = 0
        self.terminal = False
    ...
    def _buy_contract(self, ind, action) -> int:
        ...
        if self.state[ind + 1] > float(0):  # buying for current step closing price
            ...
            money_per_contract = self.state[ind + 1] + self.margins[ind] + self.bid_ask[ind] * self.contract_size[
                ind] + self.commission
    ...
    def _sell_contract(self, ind, actions) -> int:
        ...
        if self.state[ind + 1] > 0:  # selling for current step closing price
            if self.contract_positions[ind] > float(0):
                ...
                money_per_contract = self.state[ind + 1] + self.margins[ind] - self.bid_ask[
                    ind] * self.contract_size[ind] - self.commission
                costs_per_contract = self.bid_ask[
                                         ind] * self.contract_size[ind] - self.commission
    ...
    def _initiate_state(self):
        if self.initial:
            state = np.array([self.initial_amount]  # cash balance
                             ...
                             + self.deposits  # balance on deposits
                             + sum([self.data[tech_ind].values.tolist() for tech_ind in self.tech_indicators], []),
                             dtype=np.float32)

        else:
            print('self.initial == False ! (_initiate_state()')
            state = np.array([self.previous_state[0]]
                             ...
                             + self.previous_state[(self.contract_dim * 2 + 1):(self.contract_dim * 2 + 3)]
                             + sum([self.data[tech_ind].values.tolist() for tech_ind in self.tech_indicators], []),
                             dtype=np.float32)
